<?php
require_once 'Sabai/Gettext.php';
...
class Plugg_Gettext extends Sabai_Gettext
{
    ...
    {
        $this->_localeDir = $plugg->getConfig('localeDir');
        $this->_cache = $plugg->getLocator()->createService('Cache', array(
            // Change the cache name slightly different from the default to prevent collisions
            'cacheName' => $plugg->getLocator()->getDefaultParam('Cache', 'cacheName') . '-gettext'
        ));
