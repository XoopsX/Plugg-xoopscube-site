...
class Sabai_Gettext
{
    ...
    public function bindtextdomain($domain, $directory, $file = null)
    {
        ...
        $plural_func_expr = 'return $n > 1 ? 1 : 0;';
    ...
    {
        if ($ret = $this->dhastext($domain, $message)) {
            return $ret;
        }
        return $message;
    }
    ...
    {
        if ($message = $this->hastext($message1)) {
            if (is_array($message)) {
        ...
        return $num > 1 ? $message2 : $message1;
    }

    public function dngettext($domain, $message1, $message2, $num)
    {
        if ($message = $this->dhastext($domain, $message1)) {
            if (is_array($message)) {
    ...
    {
        return sprintf('%s/%s/%s/%s', $directory, SABAI_LANG, SABAI_CHARSET, isset($file) ? $file : $domain . '.mo');
    }
    ...
    {
        return isset($this->_messages[$this->_defaultDomain][$message]) ? $this->_messages[$this->_defaultDomain][$message] : false;
    }

    public function dhastext($domain, $message)
    {
        return isset($this->_messages[$domain][$message]) ? $this->_messages[$domain][$message] : false;
    }
}