...
abstract class Sabai_Model_EntityRepository
{
    ...
    public function fetchById($id, $noCache = false)
    {
        if ($noCache) {
            ...
            $this->_model->clearCache($this->_name, $id);
            $entity = $this->_getCollection($this->_model->getGateway($this->_name)->selectById($id))->getFirst();
        } elseif (!$entity = $this->isEntityCached($id)) {
            if (!$entity = $this->_model->isCached($this->_name, $id)) {
                $entity = $this->_getCollection($this->_model->getGateway($this->_name)->selectById($id))->getFirst();
            }
        }
    ...
    {
        return $this->_getCollection($this->_model->getGateway($this->getName())->selectByCriteria($criteria, array(), $limit, $offset, $sort, $order));
    }
    ...
    protected function _fetchByAssoc($selfTable, $assocEntity, $assocTargetKey, $id, $limit = 0, $offset = 0, $sort = null, $order = null)
    {
        ...
        return $this->_getCollection($this->_model->getGateway($assocEntity)->selectByCriteria($criteria, $fields, $limit, $offset, $sort, $order));
    }
    ...
    protected function _fetchByAssocAndCriteria($selfTable, $assocEntity, $assocTargetKey, $id, Sabai_Model_Criteria $criteria, $limit = 0, $offset = 0, $sort = null, $order = null)
    {
        ...
        $fields = array('DISTINCT ' . $selfTable . '.*');
        return $this->_getCollection($this->_model->getGateway($assocEntity)->selectByCriteria($criterion, $fields, $limit, $offset, $sort, $order));
    }
