...
abstract class Sabai_Model_Gateway
{
     ...
     */
    public function selectByIdFromMainTable($id, $fields = array())
    {
     ...
     */
    public function updateByCriteria(Sabai_Model_Criteria $criteria, $values)
    {
        $sets = array();
    ...
    public function selectBySQL($sql, $limit = 0, $offset = 0, $sort = null, $order = null, $group = null)
    {
        if (!empty($group)) {
            $fields = $this->getFields();
            if (isset($fields[$group])) $sql .= ' GROUP BY ' . $group;
    ...
    abstract protected function _getSelectByIdQuery($id, array $fields);
    abstract protected function _getSelectByCriteriaQuery($criteriaStr, array $fields);
    abstract protected function _getInsertQuery(array $values);
    abstract protected function _getUpdateQuery($id, array $values);
    abstract protected function _getDeleteQuery($id);
    abstract protected function _getUpdateByCriteriaQuery($criteriaStr, array $sets);
