...
<rdf:RDF
 ...
 <channel rdf:about="<?php echo $this->URL->create(array('path' => '/rss'));?>">
  <title><?php printf($this->_("%s's blog articles"), h($identity->getUsername()));?> | <?php _h($this->Config->get('siteName'));?></title>
  <link><?php echo $this->URL->create();?></link>
  <description><?php printf($this->_("%s's recent published blog articles"), h($identity->getUsername()));?></description>
  <items>
