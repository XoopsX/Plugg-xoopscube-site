...
pluginsy/Project/Model/LinkvoteHTMLQuickForm.php
*/
abstract class Plugg_Project_Model_Base_LinkvoteHTMLQuickForm extends Sabai_Model_EntityHTMLQuickForm
{
    public function onInit(array $params)
    {
        $this->addElement('altselect', 'rating', $this->_model->_('Rating'), array(
            1 => $this->_model->_('Yes'),
            0 => $this->_model->_('No'),
        ))->setDelimiter('&nbsp;');
        $this->addElement('selectmodelentity', $this->_model, 'Link', 'Link', $this->_model->_('link'), null, array('size' => 1));
    ...
    {
        $defaults = array();
        foreach (array('rating') as $key) {
            if ($this->elementExists($key) || $this->isInGroup($key)) {
                $defaults[$key] = $entity->getVar($key);
            }
        }
        if ($this->elementExists('Link')) {
            $defaults['Link'] = $entity->getVar('link_id');
        }
    ...
    {
        $vars = array();
        foreach (array('rating' => 'rating', 'link_id' => 'Link', 'userid' => 'userid') as $var_name => $form_name) {
