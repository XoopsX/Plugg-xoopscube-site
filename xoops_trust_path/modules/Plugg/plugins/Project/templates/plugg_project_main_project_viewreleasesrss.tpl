...
<rdf:RDF
 ...
 <channel rdf:about="<?php echo $this->URL->create(array('path' => '/' . $project->getId() . '/releases/rss'));?>">
  <title><?php _h($project->name);?> - <?php $this->_e('Recent project releases');?> | <?php _h($this->Config->get('siteName'));?></title>
  <link><?php echo $this->URL->create(array('path' => '/' . $project->getId() . '/releases'));?></link>
...
<?php   foreach ($releases->with('Project')->with('User') as $release):?>
 <item rdf:about="<?php echo $this->URL->create(array('path' => '/release/' . $release->getId()));?>">
  ...
  <content:encoded><![CDATA[<?php echo $release->get('summary_httml');?>]]></content:encoded>
