...
class Plugg_SimpleSearch_Plugin extends Plugg_Plugin implements Plugg_Search_Engine
{
        ...
        if ($rs = $db->query($sql, $limit, $offset)) {
            ...
            while ($row = $rs->fetchAssoc()) {
                $ret[] = array(
                    ...
                    'snippet_html' => $search_plugin->highlightKeywords($search_plugin->createSnippet($row['text_body'], $keywords), $keywords),
                );
            }
        }
        return $ret;
    ...
    {
        $db = $this->getDB();
        $sql = sprintf('DELETE FROM %stext WHERE text_searchable_id = %d AND text_content_id = %d', $db->getResourcePrefix(), $searchableId, $contentId);
        return $db->exec($sql, false);
    }
    ...
    {
        $db = $this->getDB();
        $sql = sprintf('DELETE FROM %stext WHERE text_searchable_id = %d AND text_content_group LIKE %s', $db->getResourcePrefix(), $searchableId, $db->escapeString($contentGroup . '%'));
        return $db->exec($sql, false);
    }
