...
pluginsy/Widget/Model/AuthGateway.php
*/
abstract class Plugg_Widget_Model_Base_AuthGateway extends Sabai_Model_Gateway
{
    ...
    protected function _getSelectByIdQuery($id, array $fields)
    {
        ...
        return sprintf('SELECT t.%s FROM %sauth t WHERE auth_id = %d', $fields, $this->_db->getResourcePrefix(), $id);
    }

    protected function _getSelectByCriteriaQuery($criteriaStr, array $fields)
    {
        $fields = empty($fields) ? '*' : implode(', t.', $fields);
        return sprintf('SELECT t.%1$s FROM %2$sauth t WHERE %3$s', $fields, $this->_db->getResourcePrefix(), $criteriaStr);
    }
