...
pluginsy/Widget/Model/AuthdataGateway.php
*/
abstract class Plugg_Widget_Model_Base_AuthdataGateway extends Sabai_Model_Gateway
{
    ...
    protected function _getSelectByIdQuery($id, array $fields)
    {
        ...
        return sprintf('SELECT t.%s FROM %sauthdata t WHERE authdata_id = %d', $fields, $this->_db->getResourcePrefix(), $id);
    }

    protected function _getSelectByCriteriaQuery($criteriaStr, array $fields)
    {
        $fields = empty($fields) ? '*' : implode(', t.', $fields);
        return sprintf('SELECT t.%1$s FROM %2$sauthdata t WHERE %3$s', $fields, $this->_db->getResourcePrefix(), $criteriaStr);
    }
