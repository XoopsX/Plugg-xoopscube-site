...
abstract class Plugg_XOOPSCube_Model_Base_BlockGateway extends Sabai_Model_Gateway
{
    ...
    protected function _getSelectByIdQuery($id, array $fields)
    {
        ...
        return sprintf('SELECT %s FROM %sblock WHERE block_id = %d', $fields, $this->_db->getResourcePrefix(), $id);
    }

    protected function _getSelectByCriteriaQuery($criteriaStr, array $fields)
    {
        $fields = empty($fields) ? '*' : implode(', ', $fields);
        return sprintf('SELECT %1$s FROM %2$sblock WHERE %3$s', $fields, $this->_db->getResourcePrefix(), $criteriaStr);
    }
