...
<?php   foreach ($module['results'] as $result):?>
  <li>
    ...
    <small>(<?php echo $this->Time->ago($result['time']);?>)</small>
  </li>
<?php   endforeach;?>
</ul>
<?php if ($module['has_more'] || count($module['results']) == 5):?><a href="<?php echo $search_url;?>?action=showallbyuser&mid=<?php echo $module_id;?>&uid=<?php echo $identity->getId();?>"><?php $this->_e('Show all');?></a><?php endif;?>
<?php endforeach;?>