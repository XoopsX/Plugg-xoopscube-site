...
<rdf:RDF
 ...
 <channel rdf:about="<?php echo $this->URL->create(array('path' => '/rss/' . $route));?>">
  <title><?php _h($sitename);?> (<?php printf($this->_('tag: %s'), h($tag->name));?>)</title>
  <link><?php echo $this->URL->create(array('base' => $route));?></link>
...
<?php if (isset($nodes)):?>
<?php   foreach ($nodes->with('Tags')->with('User')->with('Category') as $node):?>
