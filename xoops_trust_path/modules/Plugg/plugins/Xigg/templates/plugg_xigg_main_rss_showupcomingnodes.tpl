...
<rdf:RDF
 ...
 <channel rdf:about="<?php echo $this->URL->create(array('path' => '/rss/upcoming'));?>">
  <title><?php _h($sitename);?><?php if(isset($requested_category)):?> (<?php _h($requested_category->name);?>)<?php endif;?></title>
  <link><?php if(!isset($requested_category)):?><?php echo $this->URL->create(array('path' => '/upcoming', 'params' => array('keyword' => $requested_keyword)));?><?php else:?><?php echo $this->URL->create(array('path' => '/upcoming', array('category_id' => $requested_category->getId(), 'keyword' => $requested_keyword)));?><?php endif;?></link>
  <description><?php $this->_e('Recent upcoming news entries');?></description>
  <items>
<?php if (isset($nodes)):?>
   <rdf:Seq>
